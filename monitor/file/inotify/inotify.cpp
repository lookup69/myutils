...
// static
void Inotify::DumpInotifyEvent(struct inotify_event *i)
{
        printf("    wd =%2d; ", i->wd);
        if (i->cookie > 0)
...
{
        struct epoll_event events[EPOLL_EVENT_LEN] = { 0 };
        ...
        m_bRuning = true;
        while (!m_bExit) {
                int nfds = epoll_wait(m_epollFd, events, 1, 1000);  // 1sec
                ...
                if (events[0].data.fd == m_inotifyFd) {
                        ...
                        for (p = buf; p < (buf + numRead);) {
                                ...
                                // Debug
                                Inotify::DumpInotifyEvent(event);

                                for (auto &it : m_observerVec)
                                        it->OnInotifyEvent(this, event, m_watchMap[event->wd]);
